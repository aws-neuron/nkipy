...
from .paged_cache import load_k_tile_from_cache
from .utils import PF_transpose_with_PE, broadcast_partition_with_PE
...
def _flash_attention_core(
    ...
    m_current_neg = nisa.tensor_scalar(
        max_, nl.maximum, m_previous, op1=nl.multiply, operand1=-1
    )
...
def _flash_attention_core_kq_matmul(
    ...
    num_k_tile = num_loads * block_size
    k = k_load_buffer.reshape((B_D_SIZE, MULTI_BUFFER, num_k_tile, B_P_SIZE))[
        :, large_tile_idx % MULTI_BUFFER, :, :
    ]
    kq_res_buf = nl.full(
...
def _active_attention_core_batched(
    ...
    The results are stored in the following three buffers
    olm_buffer: (par_dim(TILE_SIZE), q_h_per_k_h, B_D_SIZE + 2)
    """
    qk_psum = nl.ndarray(
    ...
    for i_q_h in nl.affine_range(q_h_per_k_h):
        ...
        qk_mul[...] = nisa.tensor_tensor(
            q[:, i_q_h], k, nl.multiply, dtype=qk_mul.dtype
        )
    ...
    o_previous_scaled = nl.ndarray(
        (par_dim(TILE_SIZE), q_h_per_k_h, B_D_SIZE), dtype=olm_buffer.dtype
    )
    max_ = qk_psum

    m_previous = nl.ndarray((par_dim(TILE_SIZE), q_h_per_k_h, 1), dtype=acc_type)
    m_previous[:, :, 0] = nl.copy(olm_buffer[:, :, B_D_SIZE + 1])
    olm_buffer[:, :, B_D_SIZE + 1] = nl.maximum(m_previous, max_)

    alpha = nl.ndarray((par_dim(TILE_SIZE), q_h_per_k_h, 1), dtype=acc_type)
    # ------------------ code below leads to tiny mismatch -----------------
    # for i_q_h in nl.affine_range(q_h_per_k_h):
    #     alpha[:, i_q_h, :] = nisa.activation(
    #         np.exp,
    #         m_previous[:, i_q_h, :] - m_buffer[:, i_q_h, :],
    #         scale=1.0,
    #     )
    # ------------------ code above leads to tiny mismatch -----------------
    bias = nl.zeros((par_dim(TILE_SIZE), 1), dtype=m_previous.dtype)
    alpha[...] = nisa.activation(
        np.exp,
        m_previous - olm_buffer[:, :, nl.ds(B_D_SIZE + 1, 1)],
        scale=1.0,
        bias=bias,
    )
    o_previous_scaled[...] = nl.multiply(olm_buffer[:, :, nl.ds(0, B_D_SIZE)], alpha)
    ...
    for i_q_h in nl.affine_range(q_h_per_k_h):
        p_local[:, i_q_h, :] = nisa.activation_reduce(
            np.exp,
            qk_psum[:, i_q_h, :],
            bias=-1 * olm_buffer[:, i_q_h, B_D_SIZE + 1],
            scale=1.0,
            reduce_op=nl.add,
            reduce_res=ps[:, i_q_h, :],
            dtype=kernel_dtype,
    ...
    olm_buffer[:, :, nl.ds(0, B_D_SIZE)] = nl.add(o_previous_scaled, pv_sbuf)

    l_prev = olm_buffer[:, :, nl.ds(B_D_SIZE, 1)] * alpha
    olm_buffer[:, :, B_D_SIZE] = l_prev + ps
