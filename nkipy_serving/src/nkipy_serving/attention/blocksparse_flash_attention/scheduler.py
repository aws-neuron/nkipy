...
class FlashTilePlan:
    ...
    def build_tile_block_tables(self, block_tables, skip_value):
        # Ensure 1D even when block_tables has a single element.
        block_tables_1d = np.asarray(block_tables).reshape(-1)
        block_tables = np.concatenate(
            [block_tables_1d, np.array([skip_value], dtype=np.int32)]
        )
    ...
    def build_tile_update_indices(self, max_num_q_tiles, q_skip_value=100_000_000):
        tile_q_offsets = self.tile_q_indices[: self.num_real_tiles, :1].flatten().copy()
        sorted_q_offsets, num_tiles_per_seq = np.unique(
            tile_q_offsets, return_counts=True
        )
        last_tile_load_indices = np.cumsum(num_tiles_per_seq) - 1
        assert len(last_tile_load_indices) <= max_num_q_tiles, (
            f"Number of q tiles {len(last_tile_load_indices)} is > {max_num_q_tiles=}"
        )

        update_pred = np.ones(self.num_tiles)
        update_pred[self.num_real_tiles :] = 0
        update_pred[last_tile_load_indices] = 0
        update_pred = update_pred.astype(np.uint8)

        num_real_q_tiles = len(last_tile_load_indices)

        # pad last_tile_indices to match padded_seqlen
        padded_last_tile_load_indices = np.empty(max_num_q_tiles, dtype=np.int32)
        padded_last_tile_load_indices[:num_real_q_tiles] = last_tile_load_indices
        padded_last_tile_load_indices[num_real_q_tiles:] = (
            0 if last_tile_load_indices.size == 0 else last_tile_load_indices[-1]
        )

        # write back indices (only used by decode merging with prefill)
        padded_write_back_indices = np.empty(max_num_q_tiles, dtype=np.int32)
        padded_write_back_indices[:num_real_q_tiles] = sorted_q_offsets
        padded_write_back_indices[num_real_q_tiles:] = q_skip_value

        padded_last_tile_indices = np.concatenate(
            (
                padded_last_tile_load_indices.reshape(max_num_q_tiles, 1),
                padded_write_back_indices.reshape(max_num_q_tiles, 1),
            ),
            axis=-1,
        )
        return update_pred, padded_last_tile_indices
...
def _ceil_div(a, b):
    assert b != 0
    result = (a + b - 1) // b
    if isinstance(a, np.ndarray) and a.dtype == np.int32:
...
def _get_cu_start(seqlens):
    ...
    seqlens_starts = np.concatenate(([np.int32(0)], cu_seqlen[:-1]))
...
class FlashAttentionPlanner:
    ...
    def MakeTilePlan(
        ...
        *,
        ...
        planner = cls(
            ...
            prior_context_lens=prior_context_lens,
            tile_size_q=tile_size_q,
            tile_size_kv=tile_size_kv,
            block_size=block_size,
            prompt_starts=prompt_starts,
            include_prompt_in_context=include_prompt_in_context,
    ...
    def __init__(
        ...
        *,
        ...
        if prompt_starts is None:
            self.prompt_starts = _get_cu_start(self.prompt_lens)
        else:
            assert self.prompt_lens.shape == prompt_starts.shape
            self.prompt_starts = prompt_starts
    ...
    def _plan(self):
        ...
        def _build_load_offsets(start_idx, seqlen, tile_size, indices, pad_value):
            num_tiles = int(_ceil_div(seqlen, tile_size))
            load_indices = np.arange(num_tiles * tile_size, dtype=np.int32) + np.int32(
                start_idx
            )
            load_indices[int(seqlen) :] = pad_value
        ...
        def _build_start_pos(seqlen, tile_size, indices):
            num_tiles = int(_ceil_div(seqlen, tile_size))
            # tile pos
            tile_start_pos = np.arange(num_tiles, dtype=np.int32) * np.int32(tile_size)
            # valid seq len
