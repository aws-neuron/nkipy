...
def transform_block_tables_for_indirect_load(
    ...
    if block_size_tiling_factor > 1:
        ...
        if block_tiling_iota is not None:
            ...
            offset = nisa.iota(
                nl.arange(block_size_tiling_factor)[None, None, :], dtype=nl.uint32
            )
        if num_tiles_per_partition > 1:
            if nisa.get_nc_version() == nisa.nc_version.gen3:
                ...
                broadcast_partition_with_PE(
                    offset[:, 0, :], offset_br[:, 0, :], out_in_psum=False
                )
        else:
            offset_br = offset
    ...
    for partition_id in nl.affine_range(num_partitions):
        ...
        if block_size_tiling_factor > 1:
            assert num_blocks_per_tile * block_size_tiling_factor == B_P_SIZE, (
                f"{num_blocks_per_tile=} {block_size_tiling_factor=}"
            )
            block_tables_partition = (
...
def load_k_tile_from_cache(
    ...
    # Flat 2D gather load + Tensor Engine transpose.
    # Output k_load_buffer: (par_dim(B_D_SIZE), MULTI_BUFFER, num_loads, block_size, B_P_SIZE)
    # — same transposed layout as the original dma_transpose path.
    k_flat = nl.ndarray(
        (par_dim(B_P_SIZE), block_size * B_D_SIZE), dtype=key_cache.dtype
    )
    i_p = nl.arange(B_P_SIZE)[:, None]
    i_f = nl.arange(block_size * B_D_SIZE)[None, :]
    for load_idx in nl.affine_range(num_loads):
        k_flat[i_p, i_f] = nl.load(
            key_cache[block_tables[i_p, load_idx, large_k_tile_idx], i_f],
        )
        for tb_i in nl.affine_range(block_size):
            k_t_psum = nl.ndarray(
                (B_D_SIZE, B_P_SIZE), dtype=key_cache.dtype, buffer=nl.psum
            )
            k_t_psum[...] = nisa.nc_transpose(
                k_flat[:, nl.ds(tb_i * B_D_SIZE, B_D_SIZE)],
                engine=nisa.tensor_engine,
            )
            k_load_buffer[:, large_k_tile_idx % MULTI_BUFFER, load_idx, tb_i, :] = (
                nl.copy(k_t_psum, dtype=key_cache.dtype)
            )
...
def load_v_tile_from_cache(
    ...
    assert kernel_dtype == v_load_buffer.dtype
    for load_idx in nl.affine_range(num_loads):
        i_p = nl.arange(B_P_SIZE)[:, None]
        i_f = nl.arange(block_size * B_D_SIZE)[None, :]
        v_load_buffer[
