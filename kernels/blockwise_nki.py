...
def load_down_proj_weights(
    ...
    nisa.dma_copy(
        ...
        oob_mode=oob_mode.skip,
        dge_mode=nisa.dge_mode.swdge, # FIXME: this can fix some issue with ToT compiler, need further investigation
    )
...
def load_block_hidden_states(buffer_idx, block_hidden_states, hidden_states, token_indices, compute_dtype):
    ...
    nisa.dma_copy(
        ...
        src=hidden_states[
            ...
            token_indices[nl.arange(TILE_SIZE)[:, None], buffer_idx + nl.arange(1)[None, :]],
            load_f,
        ],
        oob_mode=oob_mode.skip,
    )
...
def transpose_block_hidden_states(buffer_idx, block_hidden_states_T, block_hidden_states, H, compute_dtype):
    ...
    for h_i in nl.affine_range(h_n_tiles):
        ...
        block_hidden_states_T[
            ...
            h_i,
            nl.arange(TILE_SIZE)[None, :],
        ] = nisa.tensor_copy(src=tmp, mask=h_i * TILE_SIZE + nl.arange(TILE_SIZE)[:, None] < H)
...
def load_expert_affinities(
    ...
    expert_boardcasted = nisa.tensor_copy(
        expert_boardcasted, dtype=np.float32
    )
    ...
    nisa.dma_copy(
        dst=expert_affinities_masked[nl.arange(TILE_SIZE)[:, None], buffer_idx + nl.arange(1)[None, :]],
...
def load_expert_affinities_transposed(
    ...
    nisa.dma_copy(
        dst=expert_affinities_masked[nl.arange(T)[:, None], buffer_idx + nl.arange(1)[None, :]],
        # reshape because not support 2d indirect indexing
            ...
            for h_i in nl.affine_range(h_n_tiles):
                gate_and_up_proj_state_T[gate_or_up, i_i] += nisa.nc_matmul(
                    gup_weights_sbuf[
                        p_dim,
                        weight_buffer_idx,
                        h_i, gate_or_up * I + i_i * TILE_SIZE + f_dim,
                    ][
...
def blockwise_nki_static(
    ...
    current_expert_real = nl.zeros((1, BUFFER_DEGREE, 1), dtype=np.int32, buffer=nl.sbuf)
    current_expert_may_skip = nl.zeros((1, BUFFER_DEGREE, 1), dtype=np.int32, buffer=nl.sbuf)
    ...
    if down_bias_broadcasted_hbm is not None:
        ...
        down_bias_broadcasted = nl.ndarray((TILE_SIZE, BUFFER_DEGREE, H), dtype=compute_dtype, buffer=nl.sbuf)
    else:
    ...
    if True:
        for block_idx in nl.sequential_range(num_static_blocks):
            ...
            # B
            current_expert_may_skip[:, buffer_idx_now] = nl.load(block_to_expert[block_idx], dtype=np.int32)
            nisa.tensor_copy_predicated(
                src=current_expert_may_skip[:, buffer_idx_now, :],
                dst=current_expert_real[:, buffer_idx_now, :],
                predicate=nl.not_equal(current_expert_may_skip[0, buffer_idx_now, 0], ControlType.SKIP_DMA.value),
            )
            ...
            nisa.tensor_copy_predicated(
                ...
                dst=current_expert_real[:, buffer_idx_now, :],
                predicate=nl.equal(current_expert_may_skip[0, buffer_idx_now, 0], ControlType.SKIP_DMA.value),
            )
            ...
            # E
            transpose_block_hidden_states(buffer_idx_now, block_hidden_states_T, block_hidden_states, H, compute_dtype)
            ...
            if down_bias_broadcasted_hbm is not None:
                nisa.dma_copy(
                    ...
                    src=down_bias_broadcasted_hbm[current_expert_may_skip[0, buffer_idx_now, 0]],
                    oob_mode=oob_mode.skip,
                )
            ...
            # N
            store_block_hidden_states(
                buffer_idx_now, output, block_new, token_indices
            )
...
def blockwise_nki_tokengen_one_tile_replicated_hidden_state(
    ...
    if True:
        for block_idx in nl.sequential_range(n_blocks):
            ...
            # B
            current_expert[:, buffer_idx_now] = nl.load(block_to_expert[block_idx], dtype=np.int32)
    ...
    # N
    store_block_hidden_states(
        0, output, block_output, token_indices
    )
...
def blockwise_add_residual(
    ...
    if is_neuronpy:
        ...
        # For non-NeuronPy mode, just call the original kernel
        output = blockwise_np(
            hidden_states=hidden_states,
