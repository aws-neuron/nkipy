...
class NICEndpoint {
 public:
  ...
  inline std::vector<size_t> const& get_device_ids() const {
  ...
  bool initialize_rdma_ctx_for_gpu(
    ...
    if (device_ids.size() == 0) {
      ...
      actual_device_ids_ = device_ids;
    }

    initializeContexts(actual_device_ids_);
    LOG(INFO) << "NICEndpoint initialized with " << contexts_.size()
              << " context(s) for GPU " << gpu_index;

    for (auto dev : actual_device_ids_) {
 ...
 private:
  ...
  std::shared_ptr<RecvChannelGroup> getOrCreateRecvGroup(uint64_t rank_id) {
    ...
    auto numa_node =
        RdmaDeviceManager::instance().get_numa_node(actual_device_ids_[0]);
    {
  ...
  std::vector<size_t> actual_device_ids_;
