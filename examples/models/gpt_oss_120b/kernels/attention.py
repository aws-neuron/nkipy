...
from .attention_nki.prefill_kernel import (
    flash_attn_prefill,
    ...
    if is_nkipy:
        ...
        if is_prefill:
            ...
            if sliding_window == 0:
                ...
                # beta-3: no SPMD grid (the kernel loops over batch/kv-head
                # internally); non-tensor args are specialized from defaults.
                ...
                # sw=128 causal masks are HBM operands (beta-3 can't build numpy
                # inside a kernel body); materialize them as nkipy tensors.
                tile_0_mask = tensor_apis.zeros(
                    SW128_TILE0_MASK.shape, dtype=np.uint8
                ) + SW128_TILE0_MASK
                tile_i_mask = tensor_apis.zeros(
                    SW128_TILEI_MASK.shape, dtype=np.uint8
                ) + SW128_TILEI_MASK
                attn_nki = wrap_nki_kernel(
                    attention_prefill_sw128,
                    [q, k, v, sink, tile_0_mask, tile_i_mask],
                    is_nki_beta_3_version=True,
                    kernel_kwargs={"sliding_window": sliding_window},
                )
                attn_out = attn_nki(q, k, v, sink, tile_0_mask, tile_i_mask)
        ...
        q, k, v = qkv
        # NKIPy decode attention or Numpy attention
...
def pre_attention(
    *,
    ...
    if is_nkipy and not is_prefill:
        # nkipy decode
...
def attention_native(
    *,
    ...
    """
    Attention implemented using framework-native (nkipy or numpy) interfaces
    """
    is_prefill = start_pos is None
    ...
    # Apply softmax
    attention_weights = softmax(scores, is_nkipy=is_nkipy)
