...
debug_rank = os.environ.get("NKIPY_DEBUG_RANK")
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        default="./gpt-oss-120b-bf16",
        help="Base path of the presharded checkpoint; the runner appends "
        "'-TP{tp_size}' and reads shard_{rank}.safetensors from that directory "
        "(see scripts/openai_tensor_preparation.py and the README).",
    )
    parser.add_argument(
        "--tp_size",
        type=int,
    ...
    # Map each rank to a NeuronCore. gpt-oss-120b is memory-heavy (~30 GB/rank
    # after TP sharding, with separate prefill/decode expert copies), so on a
    # single TRN2 node the ranks must be spread out to avoid packing several
    # onto one device's HBM. GPT_OSS_CORE_STRIDE places rank r on logical core
    # r*stride (e.g. stride=2 under LNC2 gives one rank per device).
    core_stride = int(os.environ.get("GPT_OSS_CORE_STRIDE", "1"))
    os.environ["NEURON_RT_VISIBLE_CORES"] = str(dist.get_rank() * core_stride)
    ...
    if dist.get_rank() == 0:
        if os.environ.get("NKIPY_NOT_CLEAR_BUILD_CACHE") != "1":
    ...
    model = GPTOSSModel(weights, config)  # FIXME: embed context length
