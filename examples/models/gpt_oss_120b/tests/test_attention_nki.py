...
def test_attention_nki_prefill(sliding_window, dtype):
    ...
    key = _sample_tensor(shape=(seqlen, kv_head, head_size), dtype=dtype)
    value = _sample_tensor(shape=(seqlen, kv_head, head_size), dtype=dtype)
    sink = _sample_tensor(shape=(q_head, 1), dtype=dtype)
    ...
    # BSHD -> BDHS / BHDS layout expected by the kernels
    q = torch_to_numpy(
        query.view(1, seqlen, q_head, head_size).permute(0, 3, 2, 1).contiguous()
    )
    k = torch_to_numpy(
        key.view(1, seqlen, kv_head, head_size).permute(0, 2, 3, 1).contiguous()
    )
    v = torch_to_numpy(
        value.view(1, seqlen, kv_head, head_size).permute(0, 2, 3, 1).contiguous()
    )
    snk = torch_to_numpy(sink)

    # beta-3: no SPMD grid; wrap + drive via baremetal_jit.
    if sliding_window == 0:
        KV_TILE_SIZE = 2048
        mask = np.asarray(torch.tril(torch.ones(seqlen, seqlen, dtype=torch.uint8)))
        operands = [q, k, v, snk, mask]
        ...
        def call(a, b, c, d, e):
            return nki_op(a, b, c, d, e)

        out_nki = baremetal_jit(call)(*operands)
    else:
        operands = [q, k, v, snk, SW128_TILE0_MASK.copy(), SW128_TILEI_MASK.copy()]
        nki_op = wrap_nki_kernel(
            attention_prefill_sw128,
            operands,
            is_nki_beta_3_version=True,
            platform_target="trn2",
            kernel_kwargs={"softmax_scale": softmax_scale},
        )

        def call(a, b, c, d, e, f):
            return nki_op(a, b, c, d, e, f)
...
def test_attention_nki_decode(
    ...
    # beta-3: wrap the kernel (non-tensor args in kernel_kwargs, no SPMD grid)
    # and drive it inside a baremetal_jit'd fn. cache_k / cache_v are mutable and
    # returned; the kernel updates them in place.
    softmax_scale = 1.0 / (head_size**0.5)
    if tile_masks is None:
        operands = [qkv, cos, sin, key_cache, value_cache, sink, position_ids]
        nki_op = wrap_nki_kernel(
            flash_attn_decode,
            operands,
            is_nki_beta_3_version=True,
        ...
        def call(a, b, c, d, e, f, g):
            return nki_op(a, b, c, d, e, f, g)

        out_nki, updated_cache_k, updated_cache_v = baremetal_jit(call)(*operands)
    else:
        operands = [
            qkv,
            cos,
        ...
        def call(a, b, c, d, e, f, g, hmask):
            return nki_op(a, b, c, d, e, f, g, hmask)

        out_nki, updated_cache_k, updated_cache_v = baremetal_jit(call)(*operands)
    out_nki = out_nki.reshape(batch_size, q_head, head_size)
...
def rope_nki(qkv, sin, cos, position_ids, h, k_h):
    ...
    # beta-3: no SPMD grid; single kv head.
    assert k_h == 1
    kv_head_id = 0
...
# rope_nki (like flash_attn_decode) assumes a single kv head (k_h == 1).
@pytest.mark.parametrize("batch_size", [1, 8])
@pytest.mark.parametrize("q_head,kv_head", [(8, 1)])
@pytest.mark.parametrize("dtype", ["bfloat16", "float32"])
def test_decode_rope(batch_size, q_head, kv_head, dtype):
    ...
    nki_op = wrap_nki_kernel(
        ...
        is_nki_beta_3_version=True,
        platform_target="trn2",
        kernel_kwargs={"h": q_head, "k_h": kv_head},
    )

    def call(a, b, c, d):
        return nki_op(a, b, c, d)

    q_nki, k_nki, v_nki = baremetal_jit(call)(*operands)
...
def decode_update_kv_cache(
    ...
    cache_k: nt.mutable_tensor,  # [batch_size, num_heads, head_size, max_model_len]
    cache_v: nt.mutable_tensor,  # [batch_size, max_model_len, num_heads, head_size]
):
    ...
    nisa.dma_transpose(dst=k_sbuf, src=k[:, kv_head_id, :], axes=(1, 0))
...
def test_decode_update_kv_cache(batch_size, num_heads):
    ...
    # Run the kernel (beta-3 wrap + baremetal_jit; cache_k/cache_v mutable).
    operands = [position_ids, k, v, cache_k, cache_v]
    nki_op = wrap_nki_kernel(
        decode_update_kv_cache,
        operands,
        is_nki_beta_3_version=True,
        platform_target="trn2",
    )
    ...
    updated_cache_k, updated_cache_v = baremetal_jit(call)(*operands)
