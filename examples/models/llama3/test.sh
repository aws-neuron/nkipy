...
WEIGHTS_PATH="$SCRIPT_DIR/tmp_tinyllama_TP8"
TP_DEGREE=8 # Tensor parallelism

if [ ! -d "$WEIGHTS_PATH" ]; then
    echo "Weights not found. Downloading and converting..."
    (cd "$EXAMPLES_DIR" && python -m models.llama3.tensor_preparation --model-name TinyLlama/${MODEL_NAME} --world-size "$TP_DEGREE" --head-dim 64 --output-dir="$WEIGHTS_PATH")
...
(cd "$EXAMPLES_DIR" && torchrun --nproc-per-node "$TP_DEGREE" -m models.llama3.llama3 -n 500 --checkpoint "$WEIGHTS_PATH" --model TinyLlama/${MODEL_NAME})
