...
def parse_einsum_subscripts(subscripts: str, num_operands: int) -> Tuple[List[str], str]:
...
def _einsum_hlo(subscripts, *operands, dtype=None):
    ...
    if len(operands) == 1:
        return _einsum_unary(ctx, hlo_operands[0], input_specs[0], output_spec, analysis)
    elif len(operands) == 2:
        return _einsum_binary(
            ctx, hlo_operands[0], hlo_operands[1], 
            input_specs[0], input_specs[1], output_spec, analysis
        )
    else:
...
def _einsum_unary(ctx, operand, input_spec, output_spec, analysis):
    ...
    for i, idx in enumerate(input_spec):
        if idx not in output_spec:
            dims_to_reduce.append(i)
        else:
            output_dims.append((idx, i, operand.shape[i]))
...
def _einsum_binary(ctx, lhs, rhs, lhs_spec, rhs_spec, output_spec, analysis):
    ...
    lhs_contracting = [i for i, idx in enumerate(lhs_indices) if idx in contracting_dims]
    rhs_contracting = [i for i, idx in enumerate(rhs_indices) if idx in contracting_dims]
    
    lhs_batch = [i for i, idx in enumerate(lhs_indices) if idx in analysis['batch_dims']]
    rhs_batch = [i for i, idx in enumerate(rhs_indices) if idx in analysis['batch_dims']]
...
def _einsum_nary(ctx, operands, input_specs, output_spec, analysis):
    ...
    for i in range(1, len(operands)):
        ...
        intermediate_spec = ''.join(
            idx for idx in current_spec + input_specs[i] 
            if idx in remaining_indices and idx not in 
            ''.join(idx for idx in current_spec + input_specs[i] 
                   if idx in remaining_indices)[:current_spec.index(idx) 
                                                 if idx in current_spec else len(current_spec)]
        )
