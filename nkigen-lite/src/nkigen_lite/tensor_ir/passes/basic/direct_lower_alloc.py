"""Allocator: the allocation seam between tiling and codegen.
...
``Allocator`` collects both behind one named surface. It wraps a Builder and
...
``Allocator`` governs allocation only.
...
index. ``Allocator`` removes only the artificial coupling of alloc to the
