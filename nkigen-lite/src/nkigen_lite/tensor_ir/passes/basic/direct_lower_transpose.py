...
def emit_transpose(
    ...
    if _can_use_passthrough_partition(c_in, c_perm, dtype):
        ...
        for p0 in range(0, B, PARTITION_MAX):
            ...
            for rows, src_slices, dst_slices in prefix_row_segments(
                ...
                tile = alloc.load(x_hbm, src_slices, (rows, I * J), dtype)
                ...
                dst_tile = alloc.sbuf((rows, J, I), dtype)
    ...
    for src_slices, dst_slices, p_cov, f_cov in _tile_iter(
        ...
        if swap_pf:
            tile = alloc.load(x_hbm, src_slices, (f_cov, p_cov), dtype)
            transposed = nb.transpose(tile, (1, 0))
            nb.dma_copy(y_hbm, transposed, dst_slices)
        else:
            tile = alloc.load(x_hbm, src_slices, (p_cov, f_cov), dtype)
...
def emit_transpose_te(
    ...
    for src_slices, dst_slices, p_cov, f_cov in _tile_iter(
        ...
        if swap_pf:
            stat = alloc.load(x_hbm, src_slices, (f_cov, p_cov), dtype)
            eye_tile = alloc.load(eye_hbm, (DimSlice(0, f_cov), DimSlice(0, f_cov)), (f_cov, f_cov), dtype)

            psum = nb.alloc((p_cov, f_cov), DType.F32, MemorySpace.PSUM)
            nb.matmul(psum, stat, eye_tile, accumulate=False)

            out_sbuf = nb.tensor_copy(
                alloc.sbuf((p_cov, f_cov), DType.F32), psum
            )
