...
from nkigen_lite.nki_ir.ir import Builder

from nkigen_lite.tensor_ir.passes.basic.direct_lower_broadcast import (
    emit_broadcast_to,
    lower_broadcast,
)
...
def _check_emit(in_shape, out_shape, atol=1e-3):
    """Lower via the pipeline entry point ``emit_broadcast_to`` and verify on HW.

    This exercises the collapsed ``(L, B, T)`` fast path (stride-0 partition
    loads + view-aliased multi-dim stores) used by the full model lowering,
    which ``lower_broadcast`` (single-axis API) does not cover.
    """
    rng = np.random.default_rng(7)
    x = rng.standard_normal(in_shape if in_shape else ()).astype(np.float32)
    ref = np.broadcast_to(x, out_shape).copy()

    b = Builder("bcast_emit")
    x_hbm = b.add_input("x", in_shape if in_shape else (1,), DType.F32)
    y_hbm = b.add_input("y", out_shape, DType.F32)
    emit_broadcast_to(b, x_hbm, y_hbm, in_shape, out_shape, DType.F32)
    b.set_outputs({"y": y_hbm})

    kernel_fn = build_kb_kernel(b.graph)
    hw_out = {"y": np.zeros(out_shape, dtype=np.float32)}
    nb.compile_and_execute(
        kernel_fn,
        inputs={"x": x.reshape(in_shape if in_shape else (1,))},
        outputs=hw_out,
        compile_opts=nb.CompileOptions(target="trn2"),
    )
    np.testing.assert_allclose(
        hw_out["y"], ref, atol=atol, rtol=atol, err_msg="HW mismatch (emit)",
    )


class TestEmitBroadcastCollapsed:
    """Pipeline entry point: collapsed (L, B, T) fast path, real layer shapes."""

    # T == 1: F-broadcast over (L, B) — innermost-axis broadcast.
    def test_f_inner_4d(self):
        _check_emit((1, 128, 16, 1), (1, 128, 16, 128))

    def test_f_inner_3d(self):
        _check_emit((1, 128, 1), (1, 128, 1024))

    # L == 1: P-broadcast over (B, T) — leading-axis broadcast (stride-0 load).
    def test_p_leading_4d(self):
        _check_emit((1, 1, 128, 128), (1, 16, 128, 128))

    def test_p_leading_rowvec(self):
        _check_emit((1024,), (1, 128, 1024))

    def test_p_leading_partition_tiled(self):
        # B = 200 > 128 forces multiple partition tiles.
        _check_emit((1, 200), (100, 200))

    # Genuine middle broadcast (L, B, T): load-once / store-B-times.
    def test_middle_5d(self):
        _check_emit((1, 128, 8, 1, 128), (1, 128, 8, 2, 128))

    def test_middle_4d(self):
        _check_emit((1, 128, 1, 64), (1, 128, 16, 64))

    # Scalar fan-out (rank-0 input).
