module attributes {nisa.target = #nisa.target<trn2>} {
  func.func @qwen3_layer(%arg0: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg1: memref<256x1xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg2: memref<256x1xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg3: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg4: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg5: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg6: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg7: memref<1x128x64xf32, strided<[?, ?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg8: memref<1x128x64xf32, strided<[?, ?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg9: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg10: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>, %arg11: memref<256x256xf32, strided<[?, ?], offset: ?>, #nisa.mem<shared_hbm>>) -> memref<256x256xf32, #nisa.mem<shared_hbm>> attributes {nki.output_names = ["output"]} {
    ...
    // FIX: Q reshape (256,256) → (2,2,128,128)
    // Original used 128x2→2x128 transpose treating adjacent cols as different heads.
    // Adjacent cols are actually different head_dim values within the SAME head.
    // Head 0 occupies cols [0,128), head 1 occupies cols [128,256).
    // Fix: copy 128x128 blocks per (batch, head) pair — no transpose needed.
    ...
    scf.for %arg12 = %c0 to %c2 step %c1 {
      scf.for %arg13 = %c0 to %c2 step %c1 {
        ...
        %0 = arith.muli %arg12, %c128 : index
        %1 = arith.muli %arg13, %c128 : index
        nisa.dma_copy(dst<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], src<128| 128>=memref<256x256xf32, #nisa.mem<shared_hbm>> %mem_12[%0 + d0, %1 + d1], dge_mode=no_dge, oob_is_err=true) engine=dma
        nisa.dma_copy(dst<128| 128>=memref<2x2x128x128xf32, #nisa.mem<hbm>> %mem_21[%arg12 + 0, %arg13 + 0, d0, d1], src<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], dge_mode=no_dge, oob_is_err=true) engine=dma
        nisa.release %mem_78 : memref<128x128xf32, #nisa.mem<sbuf>>
      }
    }
    // FIX: K reshape — same fix as Q
    ...
    scf.for %arg12 = %c0 to %c2 step %c1 {
      scf.for %arg13 = %c0 to %c2 step %c1 {
        ...
        %0 = arith.muli %arg12, %c128 : index
        %1 = arith.muli %arg13, %c128 : index
        nisa.dma_copy(dst<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], src<128| 128>=memref<256x256xf32, #nisa.mem<shared_hbm>> %mem_15[%0 + d0, %1 + d1], dge_mode=no_dge, oob_is_err=true) engine=dma
        nisa.dma_copy(dst<128| 128>=memref<2x2x128x128xf32, #nisa.mem<hbm>> %mem_22[%arg12 + 0, %arg13 + 0, d0, d1], src<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], dge_mode=no_dge, oob_is_err=true) engine=dma
        nisa.release %mem_78 : memref<128x128xf32, #nisa.mem<sbuf>>
      }
    }
    // FIX: V reshape — same fix as Q/K
    ...
    scf.for %arg12 = %c0 to %c2 step %c1 {
      scf.for %arg13 = %c0 to %c2 step %c1 {
        ...
        %0 = arith.muli %arg12, %c128 : index
        %1 = arith.muli %arg13, %c128 : index
        nisa.dma_copy(dst<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], src<128| 128>=memref<256x256xf32, #nisa.mem<shared_hbm>> %mem_18[%0 + d0, %1 + d1], dge_mode=no_dge, oob_is_err=true) engine=dma
        nisa.dma_copy(dst<128| 128>=memref<2x2x128x128xf32, #nisa.mem<shared_hbm>> %mem_23[%arg12 + 0, %arg13 + 0, d0, d1], src<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], dge_mode=no_dge, oob_is_err=true) engine=dma
        nisa.release %mem_78 : memref<128x128xf32, #nisa.mem<sbuf>>
      }
    }
    // FIX: q_rot0 = q0 * freqs_cos - q1 * freqs_sin
    // Fused into one loop so all vector ops use 1-partition temps.
    // Original had 3 separate loops writing to 4-partition SBUF intermediates
    // (mem_24, mem_25), then a tensor_tensor_arith reading individual partitions
    // of those 4-partition tensors via engine=vector — which is illegal because
    // the vector engine processes all partitions simultaneously and cannot
    // selectively address partition N of a multi-partition SBUF tensor.
    ...
    scf.for %arg12 = %c0 to %c4 step %c1 {
      ...
      // q1 * freqs_sin
      %mem_81 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_81[d0, d1, d2], src<1| 128, 64>=memref<2x2x128x128xf32, #nisa.mem<hbm>> %mem_21[%0 + 0, %1 + d0, d1, d2 + 64], dge_mode=no_dge, oob_is_err=true) engine=dma
      %mem_82 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_82[d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, strided<[?, ?, ?], offset: ?>, #nisa.mem<shared_hbm>> %arg8[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      %mem_83 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.tensor_tensor_arith(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_83[d0, d1, d2], lhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_81[d0, d1, d2], rhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_82[d0, d1, d2], op=multiply) engine=vector
      nisa.release %mem_82 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.release %mem_81 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      // q_rot0 = q0*cos - q1*sin (both operands are 1-partition, safe for vector engine)
      %mem_84 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.tensor_tensor_arith(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_84[d0, d1, d2], lhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_80[d0, d1, d2], rhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_83[d0, d1, d2], op=subtract) engine=vector
      nisa.release %mem_83 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.release %mem_80 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<4x128x64xf32, #nisa.mem<sbuf>> %mem_26[%arg12 + d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_84[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      nisa.release %mem_84 : memref<1x128x64xf32, #nisa.mem<sbuf>>
    }
    // FIX: q_rot1 = q0 * freqs_sin + q1 * freqs_cos (same fused-loop fix)
    ...
    scf.for %arg12 = %c0 to %c4 step %c1 {
      ...
      nisa.dma_copy(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_81[d0, d1, d2], src<1| 128, 64>=memref<2x2x128x128xf32, #nisa.mem<hbm>> %mem_21[%0 + 0, %1 + d0, d1, d2 + 64], dge_mode=no_dge, oob_is_err=true) engine=dma
      %mem_82 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_82[d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, strided<[?, ?, ?], offset: ?>, #nisa.mem<shared_hbm>> %arg7[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      %mem_83 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.tensor_tensor_arith(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_83[d0, d1, d2], lhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_81[d0, d1, d2], rhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_82[d0, d1, d2], op=multiply) engine=vector
      nisa.release %mem_82 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.release %mem_81 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      ...
      %mem_84 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.tensor_tensor_arith(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_84[d0, d1, d2], lhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_80[d0, d1, d2], rhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_83[d0, d1, d2], op=add) engine=vector
      nisa.release %mem_83 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.release %mem_80 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<4x128x64xf32, #nisa.mem<sbuf>> %mem_29[%arg12 + d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_84[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      nisa.release %mem_84 : memref<1x128x64xf32, #nisa.mem<sbuf>>
    }
    ...
    // FIX: k_rot0 = k0 * freqs_cos - k1 * freqs_sin (same fused-loop fix)
    ...
    scf.for %arg12 = %c0 to %c4 step %c1 {
      ...
      // k1 * freqs_sin
      %mem_81 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_81[d0, d1, d2], src<1| 128, 64>=memref<2x2x128x128xf32, #nisa.mem<hbm>> %mem_22[%0 + 0, %1 + d0, d1, d2 + 64], dge_mode=no_dge, oob_is_err=true) engine=dma
      %mem_82 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_82[d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, strided<[?, ?, ?], offset: ?>, #nisa.mem<shared_hbm>> %arg8[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      %mem_83 = nisa.alloc alignment=64 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.tensor_tensor_arith(dst<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_83[d0, d1, d2], lhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_81[d0, d1, d2], rhs<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_82[d0, d1, d2], op=multiply) engine=vector
      ...
      nisa.release %mem_83 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.release %mem_80 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<4x128x64xf32, #nisa.mem<sbuf>> %mem_33[%arg12 + d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_84[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      nisa.release %mem_84 : memref<1x128x64xf32, #nisa.mem<sbuf>>
    }
    // FIX: k_rot1 = k0 * freqs_sin + k1 * freqs_cos (same fused-loop fix)
    ...
    scf.for %arg12 = %c0 to %c4 step %c1 {
      ...
      nisa.release %mem_83 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.release %mem_80 : memref<1x128x64xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<1| 128, 64>=memref<4x128x64xf32, #nisa.mem<sbuf>> %mem_36[%arg12 + d0, d1, d2], src<1| 128, 64>=memref<1x128x64xf32, #nisa.mem<sbuf>> %mem_84[d0, d1, d2], dge_mode=no_dge, oob_is_err=true) engine=dma
      nisa.release %mem_84 : memref<1x128x64xf32, #nisa.mem<sbuf>>
    }
    ...
    // FIX: head-concat reshape: (4,128,128) → (128,2,2,128) for matmul stationary.
    // Original used a broken 2-step transpose via mem_49 (2,128,2,128 sbuf) where
    // the DMA transpose tile <128|2> tried to write 128 partitions to a 2-partition
    // tensor — out of bounds.
    // Fix: skip mem_49, directly transpose each (seq,hd) block into mem_51.
    // mem_51[hd, batch, head, seq] = mem_48[batch*2+head, seq, hd]
    ...
    scf.for %arg12 = %c0 to %c4 step %c1 {
      %mem_78 = nisa.alloc alignment=64 : memref<128x128xf32, #nisa.mem<sbuf>>
      nisa.dma_copy(dst<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], src<128| 128>=memref<4x128x128xf32, #nisa.mem<shared_hbm>> %mem_48[%arg12 + 0, d0, d1], dge_mode=no_dge, oob_is_err=true) engine=dma
      %0 = arith.divui %arg12, %c2 : index
      %1 = arith.remui %arg12, %c2 : index
      nisa.dma_transpose(dst<128| 128>=memref<128x2x2x128xf32, #nisa.mem<sbuf>> %mem_51[d0, %1 + 0, %0 + 0, d1], src<128| 128>=memref<128x128xf32, #nisa.mem<sbuf>> %mem_78[d0, d1], permutation=[1, 0], dge_mode=no_dge, oob_is_err=true) engine=dma
      nisa.release %mem_78 : memref<128x128xf32, #nisa.mem<sbuf>>
    }
